...
{
    if((-1 == socket_fd)||(NULL == buf)||(0 >= size)){
        PRINT_ERROR("tcp send error [fd(%d)---buf(0x%08x)---bufSize(%d)]", socket_fd, buf, size);
        return -1;
    }
...
{
    pthread_rwlock_init(&chnMgrLock, NULL);
    mChannelList.clear();
}
...
void ChnDataMgr::destroy_data_channel(int32_t socketFd)
{
    pthread_rwlock_wrlock(&chnMgrLock);
    if(mChannelList.empty()){
        PRINT_ERROR("channel List is empty ...");
        pthread_rwlock_unlock(&chnMgrLock);
        return ;
    }
...
{
    bool ret = true;
    pthread_rwlock_rdlock(&chnMgrLock);
    ret = dataChannelIsFull(socketFd);
    pthread_rwlock_unlock(&chnMgrLock);
    return ret;
}
...
int32_t ChnDataMgr::send_data_toChannel(int32_t socketFd, IPC_MSG_t msg)
{
    ...
    if(-1 != fd){
        if(!dataChannelIsFull(fd)){
            ...
            // 与socketFd对应的通道已满
            PRINT_ERROR("channel (Fd:%d) is full ...", socketFd);
            pthread_rwlock_unlock(&chnMgrLock);
            return -3;
        }
    }else{
        // 未创建与socketFd对应的通道
        PRINT_ERROR("channel (Fd:%d) is not be create ...", socketFd);
        pthread_rwlock_unlock(&chnMgrLock);
        return -2;
    }
...
bool ChnDataMgr::data_channel_is_empty(int32_t socketFd)
{
    bool ret = true;
    pthread_rwlock_rdlock(&chnMgrLock);
    ret = dataChannelIsEmpty(socketFd);
    pthread_rwlock_unlock(&chnMgrLock);
    return ret;
}
...
int32_t ChnDataMgr::get_data_payloadSize(int32_t socketFd)
{
    ...
    // 找到对应的通道
    if(-1 != fd){
        if(!dataChannelIsEmpty(fd)){
            pthread_rwlock_unlock(&chnMgrLock);
            return (*iter).arrayMsg[(*iter).startIndex].msgLen;
        }else{
            // 与socketFd对应的通道为空
            PRINT_ERROR("channel (Fd:%d) is empty ...", socketFd);
            pthread_rwlock_unlock(&chnMgrLock);
            return -3;
        }
    }else{
...
bool ChnDataMgr::dataChannelIsFull(int32_t socketFd)
{
    if(mChannelList.empty()){
        PRINT_ERROR("channel List is empty ...");
        return true;
    ...
    // 未创建的socketFd通道
    PRINT_ERROR("channel [%d] is not create ...", socketFd);
    return true;
}

bool ChnDataMgr::dataChannelIsEmpty(int32_t socketFd)
{
    if(mChannelList.empty()){
        PRINT_ERROR("channel List is empty ...");
        return true;
