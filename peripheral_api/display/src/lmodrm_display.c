 ...
 */
static void drm_fb_get_bpp_depth(uint32_t format, unsigned int *depth, int *bpp)
{
    switch (format) {
    case DRM_FORMAT_C8:
...
int lmo_drmInit(struct drm_dev *dev, int pz, int oz)
{
    ...
    if (!drm_plane_set_property(fd, dev->plane_primary.p, "ZPOS", pz)) {
        printf("%s: set ZPOS property failed!\n", __func__);
        goto err_plane_overlay;
    }
    if (!drm_plane_set_property(fd, dev->plane_overlay.p, "ZPOS", oz)) {
        printf("%s: set ZPOS property failed!\n", __func__);
        goto err_plane_overlay;
    }
...
int lmo_drmCommit(struct drm_buf *buffer, int width, int height,
